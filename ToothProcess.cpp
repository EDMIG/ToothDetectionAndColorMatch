...
void ColorCodeDetection::Binary()
{
	cvThreshold(r, rBi, 100, 255, CV_THRESH_BINARY_INV/*| CV_THRESH_OTSU*/);
...
void ColorCodeDetection::CalculateToothTemplateMean()
{
	for (int i=0;i<4;i++)
	{
		...
		cout<<i<<": "<<toothTemplateMean[i].val[0]<<"	"<<toothTemplateMean[i].val[1]<<"	"<<toothTemplateMean[i].val[2]<<endl;
	}
}
...
void ToothSegmentation::Segment()
{
	...
	frontToothRect.height = frontToothRect.height*3/10;//修改只检测上牙(原来3/5)
...
//为-1时表示检测失败，需重新拍照
//code表示色标ROI，toothRoI表示牙齿ROI
int ToothColorMatch(IplImage* code,IplImage* toothRoI)
{
	int resultIndex = -1;
	IplImage* imageCode = cvCloneImage(code);
	IplImage* imageToothROI = cvCloneImage(toothRoI); 
